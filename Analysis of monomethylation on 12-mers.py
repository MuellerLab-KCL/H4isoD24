...
Code used to analyse Set8 monomethylation cascade on 12-mers.
...
for _ in range(reps):
    ...
    fractionMe0 = H4me0 / (H4me0 + (H4me1 - H4me1[zeroInd]))
    fractionMe1 = (H4me1 - H4me1[zeroInd]) / (H4me0 + (H4me1 - H4me1[zeroInd]))
...
plt.title('H4K20 monomethylation by Set8')
plt.legend()
...
def model_func(X, k1):
    t = X[:, 0]
    Me0 = X[:, 1] * np.exp(-k1 * t)
    Me1 = X[:, 2] * (1 - np.exp(-k1 * t))

    return Me0 + Me1

# Set up concatenated vectors to enable global fits.
Me0_Ind = np.concatenate([np.ones(len(times_all)), np.zeros(len(times_all))])
Me1_Ind = np.concatenate([np.zeros(len(times_all)), np.ones(len(times_all))])
X = np.column_stack([np.tile(times_all, 2), Me0_Ind, Me1_Ind])
Y = np.concatenate([fractionMe0_all, fractionMe1_all])

# Perform the nonlinear fit.
popt, pcov = curve_fit(model_func, X, Y, p0=[0.01])
...
Me1_func = lambda t: 1 - np.exp(-k1 * t)
...
plt.title(('k1 = %.' + str(ci1_digits) + 'f ± %.' + str(ci1_digits) + 'f') %
